...
if (ccheckPage == null) ccheckPage = "0";
...
if (action != null && action != "") overrideValue = true;
if (override != null && override.equalsIgnoreCase("true"))overrideValue = true;
if (KBmanager.ccheckStatus(kb.name) == CCheckStatus.ONGOING) {
    ...
    show.append("<INPUT TYPE=RADIO NAME='inferenceEngine' VALUE='EProver' onclick=\"document.getElementById('SoTPTPControl').style.display='none'\" checked>EProver<br>");
    ...
    if (!tptpWorldExists && !builtInExists) show.append("<INPUT TYPE=RADIO NAME='systemOnTPTP' VALUE='Local' disabled>Local");
    else if (tptpWorldExists) show.append("<INPUT TYPE=RADIO NAME='systemOnTPTP' VALUE='Local' onClick=\"toggleList('Local')\">Local");
    else if (builtInExists) show.append("<INPUT TYPE=RADIO NAME='systemOnTPTP' VALUE='BuiltIn' onClick=\"toggleList('BuiltIn')\">Local");
    show.append("<INPUT TYPE=RADIO NAME='systemOnTPTP' VALUE='Remote' onClick=\"toggleList('Remote')\">Remote<br>");
    String params = "";
    show.append("System: ");
    if (tptpWorldExists) {
        params = "ID=systemListLocal";
        show.append(HTMLformatter.createMenu("systemChosenLocal",defaultSystemLocal, systemListLocal, params));
    }
    params = "ID=systemListBuiltIn style='display:none'";
    show.append(HTMLformatter.createMenu("systemChosenBuiltIn", defaultSystemBuiltIn, systemListBuiltIn, params));
    params = "ID=systemListRemote style='display:none'";
    show.append(HTMLformatter.createMenu("systemChosenRemote",defaultSystemRemote, systemListRemote, params));
    show.append("</div>");
    ...
    String location = "";
    int timeout = 30;
    if (timeoutStr != null && !timeoutStr.equals("")) timeout = Integer.parseInt(timeoutStr);
    if (chosenEngine == null || chosenEngine.equals(""))
        ...
        if (location == null || location.equals("")) show.append("Cannot start consistency check as no System for System on TPTP was chosen.");
        else {
            systemChosen = request.getParameter("systemChosen" + location);
            if (systemChosen == null || systemChosen.equals("")) show.append("Cannot start consistency check as no System for System on TPTP was chosen.");
            else {
    ...
    if (ccheck) {
        ...
        if (chosenEngine.equals("SoTPTP")) show.append("Chosen system: " + systemChosen + "<br>");

...
//        submitFormula("name",((Instance) childCharacters.toArray()[i]).getName(), results.get(0).toString());
      }
...
//    submitQuery("(and (instance ?STORY Story) (attribute ?STORY IntoductionPhase))");
...
//    obtainAttributes(story);
  ...
  /**
   * Checks the current operational knowledge
   */
  private void checkOperationalKnowledge() {
//    System.out.println("Current operational knowledge:");
//    for(int i=0;i<operationalKnowledge.size();i++) {
//      System.out.println(operationalKnowledge.toArray()[i]);
//    }
  }
    ...
    for(int i=0; i<6;i++) {
      ...
      if(!(results.get(0).equals("no")||results.get(0).equals("yes"))) {
        for(int j=0;j<results.size();j++) {
          ...
          //JOptionPane.showMessageDialog(null, caseRole+" "+hashmap.get(caseRole));
          submitFormula(caseRole, instance, results.get(j).toString());
        }
      }
    }
    //higher order assertions sample
...
//            JOptionPane.showMessageDialog(null, attribute+": mul? "+results.get(0));
            if(multipleAttributes.contains(results.get(0))) {
              System.out.println("We can have multiple attributes of this type");
//              JOptionPane.showMessageDialog(null, attribute+": "+results.get(0));
              submitQuery("(or (contraryAttribute "+attribute+" ?X) (contraryAttribute ?X "+attribute+"))");
//              JOptionPane.showMessageDialog(null, "opposite of "+attribute+": "+results.get(0));
...
//                JOptionPane.showMessageDialog(null, "no conflict detected");
              }
            } 
  ...
  private void showAttributes(Instance instance) {
//    for(int i=0;i<instance.getAttributes().size();i++) {
//      System.out.println(instance.getAttributes().get(i));
//    }
  }
...
//          childCharacters.add(childCharacter);
  ...
  private void resubmitFormulas() {
    ...
    try {
      for(int i=0;i<operationalKnowledge.size();i++) {
        //submitQuery(operationalKnowledge.toArray()[i].toString());
        //if(results.get(0).equals("no")) {

...
public final class SigmaRsAuditServlet extends HttpServlet {
    ...
    public void init() {
        synchronized (getServletContext()) {
            manager = (SigmaRsAuditManager)
                getServletContext().getAttribute("sigmaRsAuditManager");

            if (manager == null) {
                Path executable = Path.of(
                    "/home/shaun/workspace/sigma-rs/target/release/sumo"
                );
    ...
    protected void doPost(
        ...
        if (kb == null) {
            writeJson(response, 404,
                "{\"success\":false,\"message\":\"Unknown KB\"}");
            return;
        }
        ...
        double thoroughness =
            parseDouble(request, "thoroughness", 1.0, 0.0001, 1.0);
        double scope =
            parseDouble(request, "scope", 2.0, 1.0, 100.0);

        boolean started = manager.start(
            kb,
            new SigmaRsAuditRunner.Options(
                timeout,
                limit,
                thoroughness,
                scope
        ...
        if (started) {
            ...
            writeJson(response, 409,
                "{\"success\":false,\"message\":\"Audit already running\"}");
        }
    }
    ...
    protected void doGet(
        ...
        if (status.startedAt() != null) {
            json.put("startedAt", status.startedAt().toString());
        }

        if (status.completedAt() != null) {
            json.put("completedAt", status.completedAt().toString());
        }

        if (status.error() != null) {
            json.put("error", status.error());
        }

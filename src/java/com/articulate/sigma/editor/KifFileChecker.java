...
public class KifFileChecker {
    ...
    public static void main(String[] args) {
        ...
        if ("-l".equals(args[0]) && args.length > 1) {
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(fname));
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    if (errors.isEmpty()) out.println("  No errors");
                    else {
                        for (ErrRec e : errors) {
        ...
        if ("-C".equals(args[0]) && args.length > 1) {
            ...
            if (errors.isEmpty()) {
                ...
                for (ErrRec e : errors) System.out.println(e.toString());
            }
            return;
        }
        if ("-c".equals(args[0]) && args.length > 1) {
            ...
            try {
                ...
                if (errors.isEmpty()) {
                    ...
                    if (debug) for (ErrRec e : errors) System.out.println(e.toString());
                }
            } 
            catch (Exception e) {
                if (debug) System.out.println("*******************************************************");
    ...
    public static List<ErrRec> check(String contents, String fileName) {
        ...
        for (Formula f : localKif.formulaMap.values()) {
            ...
            CheckExistentialInAntecedent(fileName, new Formula(f.getFormula()), formulaText, formulaStartLine, msgs);
            CheckSingleUseVariables(fileName, new Formula(f.getFormula()), formulaText, formulaStartLine, msgs);
            CheckOrphanVars(fileName, new Formula(f.getFormula()), formulaText, formulaStartLine, msgs);
            ...
            CheckIsValidFormula(fileName, new Formula(f.getFormula()), formulaStartLine, kb, formulaText, msgs);
            CheckTermsBelowEntity(fileName, new Formula(f.getFormula()), formulaStartLine, formulaText, kb, localIndividuals, localSubclasses, msgs);
        }
        SortMessages(msgs);
        ...
        if (debug) for (ErrRec e : msgs) System.out.println(e);
        return msgs;
    }
    ...
    public static String formatKif(String contents) {
        ...
        if (spans.isEmpty()) return contents;
        KIF kif = new KIF();
        ...
        if (kif.formulaMap == null || kif.formulaMap.isEmpty()) return contents;
        List<Formula> orderedFormulas = new ArrayList<>(kif.formulaMap.values());
        orderedFormulas.sort(Comparator.comparingInt(fa -> fa.startLine));
        List<String> formattedForms = new ArrayList<>();
        for (Formula f : orderedFormulas) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            int spanEnd   = span[1];
            if (cursor < spanStart) out.append(contents, cursor, spanStart);
            int nextSpanStart = (i + 1 < count) ? spans.get(i + 1)[0] : contents.length();
            int endOfLine = contents.indexOf('\n', spanEnd);
            if (endOfLine == -1) endOfLine = contents.length();
            else endOfLine = endOfLine;
            int extendedEnd = Math.min(endOfLine, nextSpanStart);
            if (extendedEnd < spanEnd) extendedEnd = spanEnd;
            String rawFormula = contents.substring(spanStart, Math.min(extendedEnd, contents.length()));
            String formatted  = formattedForms.get(i);
            if (rawFormula.indexOf(';') >= 0) out.append(rawFormula);
            else {
                if (formatted != null && !formatted.isEmpty()) out.append(formatted); 
                else out.append(rawFormula);
            }
            cursor = extendedEnd;
        }
        if (cursor < contents.length()) out.append(contents.substring(cursor));
     ...
     */
    public static Set<Expr> CheckFormulaPreprocess(String fileName, KB kb, Formula f, int formulaStartLine, List<ErrRec> msgs) {
    ...
    public static void CheckIsValidFormula(String fileName, Formula f, int formulaStartLine, KB kb, String formulaText, List<ErrRec> msgs) {
        ...
        if (!KButilities.isValidFormula(kb, codeOnly)) {
            for (String er : KButilities.errors) {
                ...
                int[] rel = {-1, -1};
                if (token != null) rel = findLineInFormula(formulaText, token);  // still use ORIGINAL text
                int absLine = (formulaStartLine > 0 ? formulaStartLine : f.startLine) + ((rel[0] >= 0) ? rel[0] : 0);
                int absCol = (rel[1] >= 0) ? rel[1] : 1;
                String[] lines = formulaText.split("\n", -1);
    ...
    public static String extractBufferSlice(String[] bufferLines, int startLine, int endLine) {
        ...
        if (startLine < 1 || endLine < startLine || startLine > bufferLines.length) return "";
        StringBuilder sb = new StringBuilder();
        for (int i = startLine - 1; i < Math.min(endLine, bufferLines.length); i++) sb.append(bufferLines[i]).append("\n");
        return sb.toString();
    }
    ...
    public static int[] findLineInFormula(String formulaText, String term) {
        ...
        if (formulaText == null || term == null || term.isEmpty()) return new int[]{-1, -1};
        String[] lines = formulaText.split("\n", -1);
        for (int i = 0; i < lines.length; i++) {
            String line = lines[i];
            int searchStart = 0;
            while (searchStart < line.length()) {
                ...
                if (validStart && validEnd) return new int[]{i, pos};
                searchStart = pos + 1;
            }
        }
        return new int[]{-1, -1};
    ...
    public static void harvestLocalFacts(Formula f, Set<String> localIndividuals, Set<String> localSubclasses) {
        ...
        if (args != null && args.size() == 2){
            ...
            if(isConst(indivOrChild) && isConst(parentClass)){
                if ("instance".equals(functor)) localIndividuals.add(indivOrChild);
                else if ("subclass".equals(functor) && args != null && args.size() == 2) {
                    localSubclasses.add(indivOrChild);
                }
            }
        }
        if (f.listP()) for (int i = 1; i < f.listLength(); i++) 
    ...
    public static ErrRec parseKifError(String raw, String fileName) {
        ...
        if (m1.find()) line = Integer.parseInt(m1.group(1));
        ...
        if (m3.find() && line == 0) line = Integer.parseInt(m3.group(1));

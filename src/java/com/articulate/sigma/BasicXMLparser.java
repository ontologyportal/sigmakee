...
public class BasicXMLparser {
    ...
    private void parse(LineNumberReader lnr, BasicXMLelement element) throws ParseException, IOException {
        ...
        String newTag;
        boolean inQuote;
        String line, endTagString, nameString, valueString;
        int tagStart, tagEnd, name, value;
        BasicXMLelement newElement;
        char valueEnd;
        while (lnr.ready()) {
            line = lnr.readLine();
            inQuote = false;
            ...
            if (line.charAt(tagStart+1) == '/') {      // Found a closing tag
                endTagString = line.substring(tagStart+2,line.indexOf('>'));
                ...
                newElement = new BasicXMLelement();
                ...
                if (line.charAt(tagEnd) == ' ') {     // The tag has attributes
                    do {
                        ...
                        while (tagEnd < line.length() && Character.isJavaIdentifierPart(line.charAt(tagEnd)))
                            tagEnd++;
                        nameString = line.substring(name,tagEnd);
                        ...
                        valueString = line.substring(value,tagEnd);

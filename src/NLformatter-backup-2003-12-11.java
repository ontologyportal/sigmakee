...
public class NLformatter {
  ...
  */
  public static String nlStmtPara(String stmt, Map phraseMap, Map termMap, String language) {
      ...
      while (st.hasMoreTokens()) {
          ...
          if (token.equalsIgnoreCase(")")) {
              ...
              //System.out.println("INFO in NLformatter.nlStmtPara(): paraExp: " + paraExp);
              //System.out.println("INFO in NLformatter.nlStmtPara(): phraser: " + phraser.toString());
  ...
  /******************************************************************
  *  This method reads all tokens from the top of a stack until it reachs "(".
  ...
  */
  ...
  private static String kifExprPara(Stack phraser, Map phraseMap, Map termMap, String language) {
      ...
      if (phraser.size() > 0 && phraser.peek().toString().equalsIgnoreCase("not")) {
          ...
          if (phraser.size() > 0 && phraser.peek().toString().equalsIgnoreCase("(")) // predicate is not
      ...
      if (pred.equalsIgnoreCase("=>") || pred.equalsIgnoreCase("$p$=>")) {
          ...
          if (!isNegMode) {
              if (termMap !=null && termMap.containsKey((String) words.get(1)))
                  sb.append((String) termMap.get((String) words.get(1)));
              else
                  sb.append(words.get(1));
              sb.append(" "+IFANDONLYIF+" ");
              if (termMap !=null && termMap.containsKey((String) words.get(0)))
                  sb.append((String) termMap.get((String) words.get(0)));
              else
  ...
  public static void main(String[] args) {
      ...
      phraseMap.put("subclass","(format tg subclass \"%1 %n ay ang &%klase ng mababa sa %2\")");
      HashMap termMap = new HashMap();
      termMap.put("Relation","relasiyon");
      System.out.println(nlStmtPara("(subclass SpatialRelation Relation)",phraseMap,termMap,"en"));
